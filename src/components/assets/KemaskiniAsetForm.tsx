// src/components/assets/KemaskiniAsetForm.tsx
...
export function KemaskiniAsetForm() {
  // ✅ Explicitly typed resolver prevents 'unknown' inference errors
  const { register, handleSubmit, formState: { errors } } = useForm<KemaskiniAsetFormData>({
    resolver: zodResolver(kemaskiniAsetSchema) as any, 
    defaultValues: {
  ...
  // ✅ Typed handler matches KemaskiniAsetFormData exactly
  const onSubmit = async (data: KemaskiniAsetFormData) => {
    console.log('✅ Validated Data Ready:', data);
    // TODO: Add Supabase insert call here
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} style={{ maxWidth: '800px', margin: '2rem auto', padding: '1.5rem' }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <input {...register('description')} placeholder="Laptop Dell Latitude" style={{ width: '100%', padding: '0.5rem' }} />
        {errors.description && <p style={{ color: 'red' }}>{errors.description.message}</p>}
      </div>

      {/* Acquisition Date - Key field causing 'unknown' error */}
      <div style={{ marginBottom: '1rem' }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <input type="number" step="0.01" min="0" {...register('original_cost')} placeholder="0.00" style={{ width: '100%', padding: '0.5rem' }} />
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <input type="number" min="1" max="50" {...register('useful_life_years')} placeholder="5" style={{ width: '100%', padding: '0.5rem' }} />
      </div>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <input {...register('location_code')} placeholder="KL, JHR" style={{ width: '100%', padding: '0.5rem' }} />
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <input {...register('custodian_department')} placeholder="Bahagian Pentadbiran" style={{ width: '100%', padding: '0.5rem' }} />
      ...
      <button type="submit" style={{ padding: '0.75rem 1.5rem', background: '#003366', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
        💾 SIMPAN DATA
      </button>
    </form>
  );

        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-[#05180d] border border-emerald-500/20 rounded-[24px] shadow-[0_16px_40px_0_rgba(0,0,0,0.85)] p-6 z-50 focus:outline-none">
          ...
          {success ? (
            ...
            <form onSubmit={handleUpload} className="flex flex-col gap-4">
              ...
              <div>
                <label className="block text-[9px] text-[#f5f7f2]/60 font-extrabold uppercase tracking-widest mb-1.5">Fail Laporan (.pdf, .xlsx, .csv, .doc)</label>
                <div 
                  onDragOver={handleDragOver}
                  onDrop={handleDrop}
                  className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-emerald-900/50 rounded-xl hover:bg-emerald-900/10 hover:border-emerald-500/50 transition-all cursor-pointer relative"
                >
                  {file ? (
                    ...
                    <div className="flex flex-col items-center gap-1 text-[#f5f7f2]/40 text-xs pointer-events-none">
                      ...
                      <span className="text-[9px] font-mono text-emerald-600/60 font-bold uppercase tracking-widest">PDF, Excel, CSV, Doc</span>
                    </div>
                  )}
                  <input 
                    type="file" 
                    accept=".pdf,.xlsx,.xls,.csv,.doc,.docx,.png,.jpg,.txt" 
